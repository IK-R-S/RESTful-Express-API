...
app.get('/', (req, res) => {
    try{
        ...
        console.log('[NOVA REQUISIÇÃO - ROOT API]: /')
    }
    catch(err){
...
app.get('/status', (req, res) => {
    try{
        ...
        console.log('[NOVA REQUISIÇÃO - STATUS]: /status')
    }
    catch(err){
...
app.get('/api/dashboard', async (req, res) => {
    try {
        ...
        // Logs
        console.log('[NOVA REQUISIÇÃO - DASHBOARD]: /api/dashboard\n')
...
app.post('/api/create-item', async (req, res) => {
    try {
        ...
        if (sessionToken) {
            const decodedUserSession = jwt.verify(sessionToken, 'chave_secreta_do_jwt');
            const userId = decodedUserSession;
            const data = req.body;
            const item = {
                "name": data['name'],
            ...
            res.status(response.status).send(response);
            console.log('[NOVA REQUISIÇÃO - CREATE ITEM]: /api/create-item; [BODY]:');
            console.log(data, '\n');
        } else {
            res.status(401).send({ "info": "acesso negado", "code": 401 });
        }
...
app.get('/api/item/:item', async (req, res) => {
    try{
        ...
        res.status(response.status).send(response)
        // Logs
        console.log(`[NOVA REQUISIÇÃO - DASHBOARD]: /api/item/${itemName}\n`)
